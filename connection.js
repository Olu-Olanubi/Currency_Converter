...
request.onload = function () {
  ...
  //Store JSON Object inside indexedDB
  ...
  if (request.status >= 200 && request.status < 400) {
	...
	for(const value in data){
			...
			el1.value = val;
			el2.value = val;
			fromSelect.appendChild(el1);
			toSelect.appendChild(el2);
	}
...
function calculate(){
	...
	const convertFrom = fromSelect.options[fromSelect.selectedIndex].value;
	const convertTo = toSelect.options[toSelect.selectedIndex].value;
	let amount = parseFloat(document.getElementById("fromCurrency").value);
	
	const query = `${convertFrom}_${convertTo}`;
	
	// Open a new connection, using the GET request on the URL endpoint
	request.open('GET', `https://free.currencyconverterapi.com/api/v5/convert?q=${query}&compact=y`, true);
	...
	request.onload = function () {
	  ...
	  if (request.status >= 200 && request.status < 400) {
			...
			const val = datas[query].val;
            
            const total = val * amount;
             document.getElementById("toCurrency").value = Math.round(total * 100) / 100;
